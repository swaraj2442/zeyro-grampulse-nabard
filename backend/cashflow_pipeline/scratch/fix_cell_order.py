...
for i, cell in enumerate(nb.get('cells', [])):
    if cell['cell_type'] == 'code':
        ...
        if "CFG =" in source or "CFG  =" in source:
            cfg_idx = i
        if "mlflow.set_experiment(CFG['experiment_name'])" in source:
            mlflow_idx = i

if mlflow_idx != -1 and cfg_idx != -1 and mlflow_idx < cfg_idx:
    print(f"MLflow cell is at {mlflow_idx}, CFG cell is at {cfg_idx}. Moving MLflow cell after CFG.")
    ...
    print(f"No fix needed. MLflow index: {mlflow_idx}, CFG index: {cfg_idx}")
