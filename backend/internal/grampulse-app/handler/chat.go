...
func NewChatHandler(aiService service.AIService, chatRepo repository.ChatRepository) *ChatHandler {
	return &ChatHandler{
		aiService: aiService,
		chatRepo:  chatRepo,
	}
...
func (h *ChatHandler) HandleChat(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		respondWithError(w, http.StatusBadRequest, "Invalid UUID format for user_id")
		return
	}
...
func (h *ChatHandler) HandleTTS(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		respondWithError(w, http.StatusInternalServerError, "TTS failed")
		return
	}
...
func (h *ChatHandler) HandleSTT(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondWithError(w, http.StatusBadRequest, "Missing 'audio' file in request")
		return
	}
	defer file.Close()
	...
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, "Failed to read audio file")
		return
	}
	...
	if err != nil {
		...
		respondWithError(w, http.StatusInternalServerError, "STT failed")
		return
	}
...
var allowedWsOrigins = map[string]bool{
	"https://officergrampulse.netlify.app":         true,
	"https://officergramwebapp.netlify.app":        true,
	"https://grampulsewebapp.netlify.app":          true,
	"https://grampulseenteprisewebapp.netlify.app": true,
	"https://enterpisegrampulsewebapp.netlify.app": true,
	"https://officergrampulsewebapp.netlify.app":   true,
...
var upgrader = websocket.Upgrader{
	CheckOrigin: func(r *http.Request) bool {
		...
		if origin == "" {
			// Non-browser clients (mobile apps) have no Origin header
			return true
		}
		...
		log.Printf("[SECURITY] WebSocket rejected from origin: %s", origin)
		return false
	},
}
