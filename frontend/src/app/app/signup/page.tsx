  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.session) {
        ...
        console.log("Signup onboarding_completed:", data.session.user.user_metadata?.onboarding_completed, "isCompleted:", isCompleted);
  ...
  return (
    <AuthLayout>
      <div className="w-full flex flex-col gap-3">
        {error && (
          <div className={`p-3 text-sm rounded-xl border text-center mb-2 ${error.includes('check your email') ? 'bg-green-50 text-green-700 border-green-100' : 'bg-red-50 text-red-600 border-red-100'}`}>
            {error}
          </div>
        )}
        ...
        {!showEmailForm ? (
          ...
          <form onSubmit={handleEmailSignup} className="flex flex-col gap-3 animate-in fade-in slide-in-from-top-2 duration-300">
            <input
              type="email"
              value={email}
